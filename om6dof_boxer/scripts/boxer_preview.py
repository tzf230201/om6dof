...
class Capture:
    def __init__(self, serial, gravity_source='auto', outputs=None):
        ...
        # Retaining too many librealsense frame objects exhausts its internal
        # pool and silently stops the stream. Four frames cover YOLOX latency.
        self.rs=rs; self.lock=threading.Lock(); self.latest=None; self.history=deque(maxlen=4); self.accel=deque(maxlen=20)
        ...
        def callback(frame):
            with self.lock:
                if frame.is_frameset():
                    ...
                    receipt_ns=(self.outputs.node.get_clock().now().nanoseconds if self.outputs else time.time_ns())
                    self.latest=frames;self.receipt_ns=receipt_ns;self.history.append((receipt_ns,frames))
    ...
    def frame(self, freeze=False, receipt_ns=None, with_receipt=False):
        with self.lock:
            accel=list(self.accel)
            if receipt_ns is None:
                frames=self.latest;selected_receipt_ns=self.receipt_ns
            else:
                match=next(((stamp,item) for stamp,item in reversed(self.history) if stamp==int(receipt_ns)),None)
                if match is None:return None
                selected_receipt_ns,frames=match
        ...
        if not freeze:return (image,selected_receipt_ns) if with_receipt else image
        ...
        if self.gravity_source=='imu':
            ...
            up=self.outputs.gravity_up(selected_receipt_ns)
        R=local_rotation(up)
        ...
        return {'image_bgr':image.copy(),'K':self.K.copy(),'points_camera':pts,
                ...
                'gravity_source':np.array(self.gravity_source),'receipt_ns':np.array(selected_receipt_ns)}

    def latest_receipt_ns(self):
        with self.lock:return self.receipt_ns
...
class Outputs:
    ...
    def observed(self,snapshot,clear_boxes=True):
        ...
        self.cloud_pub.publish(point_cloud2.create_cloud_xyz32(header,points.tolist()))
    ...
    def publish(self,result,points,masks):
        ...
        for i,(box,mask) in enumerate(zip(result['boxes'],masks)):
            ...
            source='YOLO' if box.get('label_source')=='yolo' else 'manual'
            text.scale.z=.025;text.color=marker.color;text.text=f"{box['label']} ({source}) {box['score']:.2f}"
