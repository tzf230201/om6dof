...
(p/'green_budget_diagnosis_args.json').write_text(json.dumps(args))
...
log=(p/'green_budget_diagnosis.log').open('w');proc=subprocess.Popen(args,stdout=log,stderr=subprocess.STDOUT)
try:
 ...
 while time.monotonic()-start<110:
  ...
  if (bool(plan.get('reason')) and not plan.get('reason', '').startswith('waiting_')) and 'graph' in received:
   break
 (p/'green_budget_diagnosis_result.json').write_text(json.dumps(received))
 plan=received.get('plan',{})
