...
def prepare_config(base_path, dataset_path, directory):
    """Freeze dataset bytes for this launch; change only reachability settings."""
    data = Path(dataset_path).expanduser().resolve(strict=True).read_bytes()
    ...
    if not witnesses:
        raise ValueError('Dataset has no position-found joint configurations')
    ...
    parameters.update({
        ...
        'workspace_samples_sha256': hashlib.sha256(data).hexdigest(),
