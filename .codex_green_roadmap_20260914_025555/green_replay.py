...
(p/'green_replay_args.json').write_text(json.dumps(args))
...
log=(p/'green_replay.log').open('w');proc=subprocess.Popen(args,stdout=log,stderr=subprocess.STDOUT)
try:
 ...
 while time.monotonic()-start<110:
  ...
  if (plan.get('valid') or plan.get('reason','').startswith('current_') or (time.monotonic()-start>90 and plan.get('reason'))) and 'graph' in received:
   break
 (p/'green_replay_result.json').write_text(json.dumps(received))
 plan=received.get('plan',{})
