...
This starts the compositor, RViz and a preview-only target selection GUI,
without camera, hardware, gamepad, fake
...
Use `target_gui:=false` to keep using an existing target GUI.
RViz also displays the existing V2 robot, environment and reachability graph
using isolated V2 TF, plus an explicit waiting message until a new preview arrives.
