...
log=(p/'replay.log').open('w');proc=subprocess.Popen(args,stdout=log,stderr=subprocess.STDOUT)
try:
 ...
 while time.monotonic()-start<55:
  ...
  if (plan.get('valid') or plan.get('reason','').startswith('current_') or (time.monotonic()-start>35 and plan.get('reason'))) and 'graph' in received:
   break
 (p/'replay_result.json').write_text(json.dumps(received))
 plan=received.get('plan',{})
