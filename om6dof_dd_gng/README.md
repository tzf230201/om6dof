...
- `~/environment_graph_data`: the planning representation of the DD-GNG
  graph. It preserves every stable `node_id`, world XYZ and edge. When the GUI
  selects one target class, only that class remains marked as a target in this
  planning view; other classes remain visible in `~/labels` and the colored
  MarkerArray. Consumers must use this typed topic rather than Marker colors
  or the compatibility JSON topic for planning.
...
Real execution additionally requires a measured, serial-bound D435i eye-in-hand
artifact. See [`docs/hand_eye_calibration.md`](docs/hand_eye_calibration.md).
