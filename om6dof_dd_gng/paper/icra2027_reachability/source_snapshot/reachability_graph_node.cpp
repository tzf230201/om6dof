...
double halton(std::uint64_t index, unsigned int base)
{
  double result = 0.0;
...
private:
  ...
  void loadParameters()
  {
    ...
    if (graph_method_ != "halton_prm" && graph_method_ != "gng") {
      throw std::runtime_error("graph_method must be 'halton_prm' or 'gng'");
    }
    if (sample_count_ < 2 || max_sampling_attempts_ < sample_count_ || neighbors_ < 1 ||
  ...
  bool buildGraph()
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "Built method=%s with %zu/%d nodes after %d candidate attempts; retained %zu "
      "validated edges in %zu components and cached %zu full-body edge sweeps",
      graph_method_.c_str(), nodes_.size(), sample_count_, attempts, edges_.size(),
  ...
  std::vector<double> haltonJointSample(int attempt) const
  {
    ...
    for (std::size_t variable = 0; variable < joints.size(); ++variable) {
      const double unit = halton(index, kHaltonBases[variable]);
  ...
  int sampleGngNodes()
  {
    ...
    parameters.max_units = static_cast<std::size_t>(sample_count_);
    ...
    const auto prototypes = reach::growingNeuralGas(normalized_training_samples, parameters);
    ...
    return sampleHaltonNodes(attempts);
  }
  ...
  void environmentCallback(const om6dof_dd_gng::msg::EnvironmentGraph::SharedPtr message)
  {
    if (!message->header.frame_id.empty() && message->header.frame_id != world_frame_) {
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 5000,
        "Ignoring environment graph in frame '%s'; expected '%s'",
    ...
    for (const auto & node : message->nodes) {
      ...
      node_by_id.emplace(node.id, std::make_pair(point, is_target));
      if (is_target) {
        targets_.push_back({node.id, point});
      } else {
    ...
    for (const auto & edge : message->edges) {
      ...
      if (source == node_by_id.end() || target == node_by_id.end() ||
        source->second.second || target->second.second)
      {
        continue;
      }
    ...
    for (const reach::Point3 & obstacle : raw_obstacle_points) {
      ...
      for (const reach::Target & target : targets_) {
    ...
    for (const reach::Segment & obstacle : raw_obstacle_segments) {
      ...
      if (!belongs_to_target_cluster) {
        obstacle_segments_.push_back(obstacle);
      }
    }
    if (initialized_) {
      rebuildExactEnvironmentScene();
