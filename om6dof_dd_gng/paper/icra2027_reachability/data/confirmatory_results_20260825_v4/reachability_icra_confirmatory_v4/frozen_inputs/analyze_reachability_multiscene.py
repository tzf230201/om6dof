...
CONFIRMATORY_PROTOCOL_ID = "icra_confirmatory_v4"
CONFIRMATORY_CATALOG_ID = "om6dof_icra_scene_catalog_v4"
...
def normalize_query(row):
    ...
    # Parse this field whenever present so the clear-scene contract works for
    # both complete v2 telemetry and focused/legacy regression fixtures.
    if "exact_replans" in row:
        result["exact_replans"] = parse_int(
            row["exact_replans"], "exact_replans", context, 0
        )
...
def audit_bundle(manifest, raw_graphs, raw_queries):
    ...
    for row in queries:
        ...
        if row["phase"] == "clear":
            ...
            require(row["blocked_nodes"] == 0,
                    f"{context}: clear phase reports blocked nodes")
            require(row["blocked_edges"] == row["exact_replans"],
                    f"{context}: clear blocked edges do not match exact replans")
        else:
