...
class AuditTest(unittest.TestCase):
    ...
    def test_clear_exact_replan_is_audited_but_unexplained_blocking_fails(self):
        rows = query_rows(True)
        clear = next(row for row in rows if row["phase"] == "clear")
        clear["blocked_edges"] = 1
        clear["exact_replans"] = 1
        result = MODULE.audit_bundle(
            manifest(), csv_raw(graph_rows()), csv_raw(rows)
        )
        self.assertEqual(result["counts"]["queries"], 36)

        clear["exact_replans"] = 0
        with self.assertRaisesRegex(
            ValueError, "clear blocked edges do not match exact replans"
        ):
            MODULE.audit_bundle(manifest(), csv_raw(graph_rows()), csv_raw(rows))

        clear["exact_replans"] = 1
        clear["blocked_nodes"] = 1
        with self.assertRaisesRegex(ValueError, "clear phase reports blocked nodes"):
            MODULE.audit_bundle(manifest(), csv_raw(graph_rows()), csv_raw(rows))
