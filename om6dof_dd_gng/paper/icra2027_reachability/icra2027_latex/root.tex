...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.98\textwidth]{lazy_query_pipeline.png}
    \caption{Environment-conditioned query procedure. A semantic target selects configuration-retaining goal candidates; cached capsules provide a conservative broad phase, Dijkstra searches the enabled roadmap, and interpolated MoveIt/FCL checks accept the path or reject one edge and trigger replanning. Rejections are scoped to the current environment revision.}
...
Each request is atomic: it carries the measured six-joint start state, selected semantic target, obstacle geometry, scene identifier, and expected graph revision. Candidate goals are robot nodes whose end-effector positions lie within $r_I$ of the target, remain collision-free, and belong to the start component. Because every robot node stores its complete joint vector, distinct inverse-kinematic branches are not merged merely because their end-effector positions are close. Figure~\ref{fig:query_pipeline} shows how this workspace intersection remains coupled to configuration-space connectivity.
...
Construction proceeds in four stages. First, candidate joint vectors are rejected if they violate joint limits, kinematic validity, or the conservative self-collision predicate; accepted vectors are normalized by joint range. Second, the two anchors preserve the measured start and designated target configuration. Third, the method-specific budget is filled: pure GNG adapts 798 codebook vectors, guarded GNG allocates $\operatorname{round}(0.75\times798)=599$ nodes to deterministic low-discrepancy guards and the remaining 199 to GNG prototypes, and Halton/PRM directly retains 798 low-discrepancy configurations. The guard indices span the accepted candidate stream rather than taking only its prefix, protecting sparse local support near target regions.

Fourth, the original learned GNG adjacency is deliberately not used. For every method, the implementation proposes the ten nearest neighbors in normalized joint distance, rejects pairs beyond $d_Q=0.75$ or 0.14~m end-effector displacement, and accepts an undirected edge only after interpolated validity checks. Thus Fig.~\ref{fig:roadmap_construction} compares node construction under a shared connector; it does not claim that the GNG topology itself is preserved.

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.98\textwidth]{roadmap_construction.png}
    \caption{Method-specific node construction in a two-dimensional analogue of normalized configuration space, produced by the same deterministic GNG and digit-permuted Halton implementations the system deploys. Marker counts are scaled; labels give the deployed 800-node composition. Pure GNG quantizes the 4,000-sample validity-filtered candidate stream, guarded GNG reserves 599 globally supportive low-discrepancy samples, and Halton/PRM samples directly. Because the goal test is a workspace one, its configuration-space preimage is disjoint, so both inverse-kinematic branches of a single target are drawn. All methods subsequently use the same validated $k$-nearest-neighbor connector.}
    \label{fig:roadmap_construction}
\end{figure*}
...
The validity hierarchy is intentional. Capsule tests are fast and conservative, so they remove obviously affected nodes and edges after an obstacle update. Dijkstra then searches only the remaining graph. Exact validation interpolates every edge of the proposed route at the finer step and checks detailed robot meshes against self and environment geometry. On failure, only the first rejected roadmap edge is blacklisted for that environment revision before search resumes; the persistent roadmap is unchanged. A route is published only after every interpolated state passes the authoritative check, and publication remains on a private preview topic.
