...
(p/'workspace_replay_args.json').write_text(json.dumps(args))
...
args += ['-p','graph_method:=workspace_samples','-p','workspace_samples_file:=/home/kublab/ros2_ws/src/om6dof/experiments/cartesian_workspace/results/comparison_v1_v2_25mm_20260909/v2/points.csv','-p','sample_count:=15175','-p','target_refinement_enabled:=false']
...
log=(p/'workspace_replay.log').open('w');proc=subprocess.Popen(args,stdout=log,stderr=subprocess.STDOUT)
try:
 ...
 while time.monotonic()-start<110:
  ...
  if (plan.get('valid') or plan.get('reason','').startswith('current_') or (time.monotonic()-start>90 and plan.get('reason'))) and 'graph' in received:
   break
 (p/'workspace_replay_result.json').write_text(json.dumps(received))
 plan=received.get('plan',{})
