...
class GraphPickNode(Node):
    def __init__(self) -> None:
        ...
        self.create_subscription(
            String, str(self.get_parameter("operation_mode_state_topic").value),
            self._on_operation_mode, state_qos, callback_group=cb)
        self.create_subscription(
            Bool, str(self.get_parameter("remote_enabled_state_topic").value),
            self._on_remote_enabled, state_qos, callback_group=cb)
        self.create_subscription(
    ...
    def _on_operation_mode(self, message: String) -> None:
        with self._lock:
            self._operation_mode = message.data.strip().upper()

    def _on_remote_enabled(self, message: Bool) -> None:
        with self._lock:
            self._remote_enabled = bool(message.data)
    ...
    def _interlock_blockers(self, plan: Optional[FrozenPickPlan] = None) -> List[str]:
        blockers = []
        operation_topic = str(
            self.get_parameter("operation_mode_state_topic").value)
        remote_topic = str(self.get_parameter("remote_enabled_state_topic").value)
        health_topic = str(self.get_parameter("dynamixel_health_topic").value)
        try:
            if self.count_publishers(operation_topic) != 1:
                blockers.append("operation_mode_publisher_count_not_one")
            if self.count_publishers(remote_topic) != 1:
                blockers.append("remote_state_publisher_count_not_one")
        ...
        if operation_mode != "AUTONOMOUS":
            blockers.append(f"operation_mode_not_autonomous:{operation_mode}")
        if remote_enabled is not False:
            blockers.append(f"remote_control_not_disabled:{remote_enabled}")
    ...
    def _build_plan(self) -> FrozenPickPlan:
        ...
        if reachability.reason != "path_ready_exact_validated_preview_only":
        ...
        if bool(self.get_parameter("require_calibration_verified").value) \
            ...
            blockers.append("camera_calibration_not_verified")
        if not self.execution_enabled:
            blockers.append("execution_disabled_at_launch")
        class_name = labels.get(target_id, "") if now - labels_time <= max_age else ""
        if not class_name:
            selected = str(perception.get("semantic_target_classes", "")).strip()
            class_name = selected if selected and selected != "all" else f"class_id:{class_id}"
    ...
    def _on_plan(self, _request, response):
        ...
        try:
            ...
            self._publish_status()
            response.success = False
            response.message = f"graph pickup plan rejected: {error}"
            return response
        with self._lock:
            self._frozen = plan
    ...
    def _execution_rejection(self, plan: FrozenPickPlan) -> Optional[str]:
        if self._faulted:
            return "motion_faulted; restart the graph-pick node after checking hardware"
        if not self.execution_enabled:
        ...
        if current.fingerprint != plan.fingerprint:
            return "live_reachability_plan_changed; request a new preview"
        return None
    ...
    def _on_execute(self, _request, response):
        ...
        with self._lock:
            ...
            self._message = f"starting graph pickup plan {plan.fingerprint[:16]}"
        threading.Thread(
