"""DBL-GNG on depth-derived 3D world points, using the existing robot TF tree."""
...
def generate_launch_description():
    ...
    mapper = Node(
        ...
        parameters=[params_file, {
            ...
            "dbl_edge_cut_period_frames": ParameterValue(
                LaunchConfiguration("edge_cut_period_frames"), value_type=int),
        }],
        remappings=inputs,
    )
    rviz = Node(
        ...
        name="dbl_gng_rviz",
        output="screen",
        arguments=["-d", str(share / "rviz" / "dbl_gng.rviz"), "-f", world_frame],
    ...
    declarations = [
        ...
        DeclareLaunchArgument(
            "edge_cut_period_frames", default_value="10",
            description="Cut weak DBL-GNG edges every N valid depth frames after node capacity is first reached."),
        DeclareLaunchArgument(
            "params_file", default_value=str(share / "config" / "dbl_gng.yaml"),
            description="ROS parameter file for the camera and DBL-GNG mapper nodes."),
    ]
    ...
    handlers = [
        ...
        for process, reason in [
            (camera, "DBL-GNG camera process exited"),
            (mapper, "DBL-GNG mapper process exited"),
            (rviz, "DBL-GNG RViz window closed"),
        ]
    ]
